...
async function test() {
  ...
  const blockHash =
    '0x0c08e75286b26c03419ab6015844b25b90b8f178d9c457b6b75832a959392014';

  const [placeholder, blockInfo]: any = await Promise.all([
    api?.rpc.chain.getBlock(blockHash),
    api?.derive.chain.getBlock(blockHash),
  ]);
  const extrinsics = blockInfo.extrinsics;
  ...
  const res = await api.rpc.payment.queryInfo(
    extrinsics[0].extrinsic.hash.toJSON()
  );
