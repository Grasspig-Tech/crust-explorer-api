...
async function test() {
  ...
  const aaa = await api.query.system.account.entries();
  const mapaaa = aaa.map(it => {
    return [it[0].toHuman(), it[1].toJSON()];
  });
  const tmpFilter = filterRepeatData;
  const wqe = filterRepeatData(mapaaa.map((it: any) => it[0][0]));
