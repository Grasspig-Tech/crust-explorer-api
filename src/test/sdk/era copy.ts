...
async function test() {
    ...
    api.rpc.chain.subscribeFinalizedHeads(async res => {
        console.log(res)
        let aaaNum = (await api?.query.system.number()).toJSON()
        let bbbNum: any = res.toJSON().number;
        // debugger;
        let [iiq, ...qwi]: any = await Promise.all([
        ...
        debugger
        let uui = await api.query.session.currentIndex.at('0xcc02b5b2ae6cc97f6a63e24aa38cd1e3684da709bb9ac1defae33301f7451394')
        let aqo = await api.query.imOnline.authoredBlocks(uui.toJSON(), "5DvTCFuPUXb6Vzg5NB4mFu3Hz95GUqXTMLXib66e4KhF9wxW")
        console.log(uui)
        console.log(aqo)
        // for (let item of qwi) {
