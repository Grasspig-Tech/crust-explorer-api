...
import { CeBlock, CeEvent, CeTransfer, CeExtrinsic, CeRewardSlash } from "../../model"
import { CONFIRM_BLOCK } from "../../config"
import { Block, Status } from "../../interface"
import { removeDot, trillionCruFormat, isExtrinsicSuccess, getAccountDisplay, getSortIndex, getBlockTimestamp } from "../../util"
...
export async function queryOneBlockByBlockNum(blockNum: number, api: ApiPromise): Promise<Block> {
    ...
    try {
        // debugger;
        // debugger;
        let blockHash: string = String((await api.rpc.chain.getBlockHash(blockNum) as any).toHuman());
        if (!blockHash) {
        ...
        const { parentHash, stateRoot, extrinsicsRoot }: { parentHash: string, stateRoot: string, extrinsicsRoot: string } = blockHeader;
        ...
        /* 
            ...
            specVersion: 运行时版本
        */
        let [lastFinalizedBlockHash, authorInfo, specVersion] = await Promise.all([
            api?.rpc.chain.getFinalizedHead().then((res: any) => res.toHuman()),
            api.derive.accounts.info(author),
            api.rpc.state.getRuntimeVersion(blockHash).then((res: any) => Number(res.toJSON().specVersion))
        ])
        ...
        let finalizedBlock: any = await api.rpc.chain.getBlock(lastFinalizedBlockHash as string)
        ...
        const ceExtrinsicLists: CeExtrinsic[] = extrinsics.map((item: any, curIndex: number): CeExtrinsic => {
            ...
            // debugger;
            const params = args?.map((it: any, curIndex: number) => ({ ...it, value: argsValues[curIndex] }));
            // debugger;
        ...
        /* 
            统一获取extrinsic中transfer字段
                ...
                暂时发现subscan的交易中callModule为balances  且callModuleFunction为transferKeepAlive或transfer
                    才有transfer字段
        */
        const ceExtrinsicListParams: { to: string, amount: number, index: number, from: string }[] =
            ceExtrinsicLists
                .map((it, index) => ({ from: it.accountId, params: JSON.parse(it.params), index, callModule: it.callModule, callModuleFunction: it.callModuleFunction }))
                .filter(it => it.callModule === "balances" && ['transferKeepAlive', 'transfer'].includes(it.callModuleFunction))
                .map(it => ({
                    from: it.from,
                    to: it.params.find((it: { name: string, type: string, value: string }) => it.name === 'dest')?.value,
                    amount: it.params.find((it: { name: string, type: string, value: string }) => it.name === "value")?.value,
                    index: it.index
                }));
        /* 统一获取extrinsic中transfer里的to对应的accountDisplay */
        const toAccountDisplays = await getAccountDisplay(ceExtrinsicListParams.map(it => ({ address: it.to })), api);
        ceExtrinsicListParams.forEach((it, curIndex) => {
            let targetTransfer: { from: string, to: string, amount: string, toAccountDisplay: string }
                = {
                from: it.from,
                to: it.to,
        ...
        const extrinsicAccountDisplays = await getAccountDisplay(ceExtrinsicLists.map(it => ({ address: it.accountId })), api);
        ...
        const ceEventLists: CeEvent[] = events.map((it: any, curIndex: number): CeEvent => {
            ...
            */
            ...
            const success: Status = allpyExtrinsicIndex == undefined ? Status.Yes : ceExtrinsicLists[allpyExtrinsicIndex]?.success;
...
export async function queryBlockByBlockNums(blockNums: number[], api: ApiPromise): Promise<Block[]> {
    try {
        let pAll = blockNums.map(item => {
            return queryOneBlockByBlockNum(item, api);
        });
        let result: Block[] = await Promise.all(pAll);
