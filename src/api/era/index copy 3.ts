...
// import { queryAccount, AccountArg } from "../account"
// import {
...
//     // let tmpNodes = await api.queryMulti([
//     //     ...(nodes as any).map((it: any) => [api.query.staking.bonded, it])
//     // ]).then(res => res.reduce((prev: any, cur: any) => [...prev, cur.toJSON()], []));
...
//     // let filterRepeatAccount: AccountArg[] = [];
...
//     //     } else {
//     //         /* 找到了，设置accountType为1 */
//     //         findData.accountType = findData.accountType === 1 ? 1 : item.accountType;
//     //     }
//     // })
//     /* 获取所有账户 */
...
//     /* chainAllAccountOnlyControllId：[{it:地址,index:索引}] */
//     // let chainAllAccountOnlyControllId = (await api.queryMulti([
//     //     ...(chainAllAccountOnlyId as any).map((it: any) => [api.query.staking.bonded, it])
//     // ])).map((it) => it.toJSON());
//     let chainAllAccountOnlyControllId = await getControllerAddressByStashAddress(chainAllAccountOnlyId, api);

//     // let tmpTotalAccount = tmpCompare(chainAllAccountOnlyId, chainAllAccountOnlyControllId.filter(it => it));
//     // let totalAccountMapToAccount =
//     //     (tmpTotalAccount.arr1Only as string[]).map((it: string) => ({ address: it, accountType: 2 }))
//     //     .concat((tmpTotalAccount.arr2Only as string[]).map((it:string)=>({address:it,accountType:1}))  )
//     //     ;
//     let chainAllAccountMapToAccount = chainAllAccountOnlyId.map((item: any, index: number) => {
...
//     // const totalAccount = tmpFilter([...allAccount,])

//     // debugger;
//     // let chainAllAccountOnly = filterRepeatData([chainAllAccountOnlyStashId], ["address", "accountType"]);
//     // let qweo = await api.queryMulti([
//     //     ...(chainAllAccount as any).map((it: any) => [api.query.staking.bonded, it])
//     // ]);
//     // let qweo22 = await api.queryMulti([
//     //     ...(qweowqo as any).map((it: any) => [api.query.staking.bonded, it])
//     // ]);
//     // let qwe = qweo.map(it => it.toJSON())
//     // console.log(qwe)
//     // let totalAccount =  
