...
export async function queryPledge(
    ...
    try {
        let ceBondedPledgeList = await Promise.all(pAll)
...
export async function queryOnePledge(
    ...
    // 担保人 [{who,value}]
    const stakersGuarantees: { who: string, value: string | number }[] = queryResult22[1] as any;
    /* 验证人冻结，全网冻结，others(提名人地址和投票) */
    let own: string = pledgeInfo.own as string;
    let total: string = pledgeInfo.total as string;
    let others: { who: string, value: string }[] = pledgeInfo.others as { who: string, value: string }[];
    own = String(Number(own));
    total = String(Number(total));
    /* 提名者冻结 = 全网冻结 - 验证人冻结  */
    let bondedNominators: string = String(Number(total) - Number(own));
    /* 他人有效质押 */
    let otherActivePledge: number = 0;
    ...
    if (stakersGuarantees) {
        stakersGuarantees
            ...
            .filter(
                (target: any) => target.who.toString() === address?.toString()
            )
            .forEach((target: any) => (pledgeTotal += Number(target.value)));
    }

    let cePledge: CeBondedPledge = {
